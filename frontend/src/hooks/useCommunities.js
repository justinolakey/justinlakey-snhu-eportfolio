...
export function useCommunities(filters) {
    ...
    // Keep a ref to the latest filters so the effect can read them
    // without needing `filters` in the dependency array.
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            setError(null);
            try {
                const data = await fetchCommunities(filtersRef.current);
    ...
    // Used to trigger a manual refresh (e.g. after creating a community)
